...
def run_commands(commands: List[str], encoding: str = "utf-8") -> Tuple[List[str], List[str]]:
    ...
    """
    result = subprocess.run(commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
...
def run_command_with_grep(commands: List[str], regexp: str) -> List[str]:
    ...
    containers = subprocess.run(["grep", "-E", f"{regexp}"], input=ps.stdout, capture_output=True)
