...
def main(filename: Path, folder: Path):
...
def main2(filename: Path, folder: Path):
    out_file = folder / 'permitted.json'
    reqs = parse_for_permitted_libs(filename)
...
if __name__ == '__main__':
    ...
    main(f, output_folder)

    permitted_versions = output_folder / 'permitted_versions.json'
    f = Path('/home/luiscberrocal/adelantos/ec-d-local-payment-collector/requirements/base.txt')
